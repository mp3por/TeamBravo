...
public class MemoryFieldsIndex extends MemoryFields {
    ...
    public void indexDocument(Document doc) throws Exception {
        ...
        // Write the document's properties to the meta index.
        metadata.writeDocumentEntry(doc.getAllProperties());
        ...
        // Add the document's length to the document index.
        ((MemoryDocumentIndexFields) document).addDocument(fdpl.getDocumentLength(), ((FieldDocumentIndexEntry) fdpl.getDocumentStatistics()).getFieldLengths());

        // Keep tally on fields.
        long[] fieldcounts = new long[fieldtags.length];

        // For each term in the document:
        for (String term : fdpl.termSet()) {

            MemoryFieldsLexiconEntry le = new MemoryFieldsLexiconEntry(1, fdpl.getFrequency(term), fdpl.getFieldFrequencies(term));

            // Insert/update term in lexicon.
            int termid = lexicon.term(term, le);

            // Insert/update term posting list.
            ((MemoryFieldsInvertedIndex) inverted).add(termid, stats.getNumberOfDocuments(), fdpl.getFrequency(term), fdpl.getFieldFrequencies(term));

            // Keep tally on fields.
            int[] ffreq = fdpl.getFieldFrequencies(term);
            for (int i = 0; i < fieldcounts.length; i++)
                fieldcounts[i] += ffreq[i];
        }

        // Update collection statistics.
        stats.update(1, fdpl.getDocumentLength(), fdpl.termSet().length);
        stats.updateUniqueTerms(lexicon.numberOfEntries());
        stats.updateFields(fieldcounts);
        stats.relcaluate();
    }
