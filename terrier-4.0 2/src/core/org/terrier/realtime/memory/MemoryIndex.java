...
public class MemoryIndex extends Index implements UpdatableIndex,WritableIndex {
	protected static final Logger logger = Logger.getLogger(MemoryIndex.class);
	...
	public void indexDocument(Document doc) throws Exception {
		...
		document.addDocument(docPostings.getDocumentLength());
		...
		stats.update(1, docPostings.getDocumentLength(),
				docPostings.termSet().length);
	...
	/** FIXME */
	private DocumentPostingList docPostings;

	/** FIXME */
	private TermPipeline pipeline_first;

	/** FIXME */
	private final static String PIPELINE_NAMESPACE = "org.terrier.terms.";

	/** FIXME */
	private TermPipeline getEndOfPipeline() {
